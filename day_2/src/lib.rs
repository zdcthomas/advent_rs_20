...
pub fn how_many_old(input: Vec<String>) -> u32 {
    let mut count = 0;
    for raw in input.into_iter() {
        let pass = Password::from(raw);
        if pass.valid_old() {
            count += 1;
        }
    }
...
pub fn how_many_new(input: Vec<String>) -> u32 {
    let mut count = 0;
    for raw in input.into_iter() {
        let pass = Password::from(raw);
        if pass.valid_new() {
...
impl From<String> for Password {
    fn from(input: String) -> Self {
        ...
        let numbers_and_char = split.next().unwrap();
        let password = split.next().unwrap().strip_prefix(" ").unwrap();
        let mut num_char_split = numbers_and_char.split(' ');
        let mut nums = num_char_split.next().unwrap().split('-');
        let min = nums.next().unwrap().parse::<u32>().unwrap();
        let max = nums.next().unwrap().parse::<u32>().unwrap();
        let letter = num_char_split.next().unwrap().chars().next().unwrap();
...
impl Password {
    ...
    fn valid_new(&self) -> bool {
        let left = self.password.chars().nth((self.first - 1) as usize);
        let right = self.password.chars().nth((self.second - 1) as usize);
        match (left, right) {
            (Some(left), Some(right)) => {
                if left == self.letter {
                    return right != self.letter;
                } else if right == self.letter {
                    return true;
                }
                false
            }
            (Some(left), None) => left == self.letter,
            _ => false,
        }
        // if let Some(letter) = left {
        //     if letter == self.letter {
        //         return true;
        //     }
        //     if let Some(last) = right {
        //         return last == self.letter;
        //     }
        // }
        // false
    }
...
#[cfg(test)]
mod tests {
    use crate::{how_many_new, how_many_old, input, sample_input, Password};

    #[test]
    fn old_works() {
        assert_eq!(how_many_old(sample_input()), 2);
        assert_eq!(how_many_old(input()), 456);
    }

    #[test]
    fn new_works() {
        assert_eq!(how_many_new(sample_input()), 1);
        assert_eq!(how_many_new(input()), 308);
    }

    #[test]
    fn from_string_for_password() {
        let input = "10-11 g: blpbhgjcgmgg".to_owned();
        assert_eq!(
            Password::from(input),
            Password {
                first: 10,
                second: 11,
                letter: 'g',
                password: "blpbhgjcgmgg".to_owned()
            }
        )
    }

    #[test]
    fn password_valid_true() {
        ...
        assert_eq!(password.valid_old(), true);
    }

    #[test]
    fn password_valid_false() {
        let password = Password {
            first: 10,
            second: 11,
            letter: 'g',
        ...
        assert_eq!(password.valid_old(), false);
    }

    #[test]
    fn new_password_valid_true() {
        let password = Password {
            first: 1,
            second: 3,
            letter: 'a',
